...
using Content.Server._NF.Nutrition.Components;
...
namespace Content.Server._NF.Nutrition.EntitySystems;

public sealed class SliceableFoodRandomSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<SliceableFoodRandomComponent, InteractUsingEvent>(OnInteractUsing);
        SubscribeLocalEvent<SliceableFoodRandomComponent, SliceFoodDoAfterEvent>(OnSlicedoAfter);
        SubscribeLocalEvent<SliceableFoodRandomComponent, ComponentStartup>(OnComponentStartup);
    }

    private void OnInteractUsing(Entity<SliceableFoodRandomComponent> entity, ref InteractUsingEvent args)
    {
        if (args.Handled)
            return;
    ...
    private void OnSlicedoAfter(Entity<SliceableFoodRandomComponent> entity, ref SliceFoodDoAfterEvent args)
    {
    ...
    private bool TrySliceFood(EntityUid uid,
        EntityUid user,
        EntityUid? usedItem,
        SliceableFoodRandomComponent? component = null,
    ...
    public EntityUid Slice(EntityUid uid,
        EntityUid user,
        SliceableFoodRandomComponent? comp = null,
        TransformComponent? transform = null)
    {
    ...
    private void OnComponentStartup(Entity<SliceableFoodRandomComponent> entity, ref ComponentStartup args)
    {
...
/*
// Content.Server/Food/Systems/CuttableFoodSystem.cs
public sealed class CuttableFoodSystem : EntitySystem
{
    [Dependency] private readonly IRobustRandom _random = default!;
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<CuttableFoodComponent, InteractUsingEvent>(OnInteractUsing);
    }
    
    private void OnInteractUsing(EntityUid uid, CuttableFoodComponent component, InteractUsingEvent args)
    {
        // Check if the used item has a sharp tool quality (like a knife)
        if (!EntityManager.TryGetComponent<ToolComponent>(args.Used, out var tool) ||
            !tool.Qualities.Contains("Sharp"))
        {
            return;
        }
        
        // Prevent cutting already cut food
        if (component.IsCut)
            return;
        ...
        // Play cutting sound
        _audioSystem.PlayPvs(component.CutSound, uid);
        
        // Determine how many pieces to spawn
        var pieceCount = _random.Next(component.MinPieces, component.MaxPieces + 1);
        
        // Spawn the pieces
        for (int i = 0; i < pieceCount; i++)
        {
            var piecePrototype = _random.Pick(component.PossiblePieces);
            var piece = Spawn(piecePrototype, Transform(uid).Coordinates);
            
            // Add some random offset for visual effect
            var randomOffset = new Vector2(_random.NextFloat(-0.2f, 0.2f), _random.NextFloat(-0.2f, 0.2f));
            Transform(piece).LocalPosition += randomOffset;
        }
        
        // Delete the original food item
        Del(uid);
...
public sealed class MysterySliceableSystem : EntitySystem
{
    [Dependency] private readonly IRobustRandom _random = default!;
    ...
    {
        base.Initialize();
        SubscribeLocalEvent<MysterySliceableComponent, SliceFoodEvent>(OnSlice);
    }

    private void OnSlice(EntityUid uid, MysterySliceableComponent comp, ref SliceFoodEvent args)
    {
        if (args.Handled || comp.PossibleSlices.Count == 0)
            return;

        // pick one random prototype from the list
        var proto = _random.Pick(comp.PossibleSlices);
        var ent = EntityManager.SpawnEntity(proto, Transform(uid).Coordinates);

        // mark the slice event handled so normal slicing doesn't run
        args.Handled = true;
        args.SliceEntity = ent;
    }
*/
}
